...
const modelList = require
  .context("./models/", false, /$/)
  .keys()
  .map((x) => {
    return x.match(/\w+/)[0];
  });
...
for (let i = modelList.length - 1; i >= 0; i--) {
  ...
  modelComponents[key] = defineAsyncComponent(() =>
    import("./models/" + key + ".vue")
  );
}
  ...
  methods: {
    ...
    savePatient() {
      ...
      const nowString = now
        ...
        .replaceAll(".", "");
      const filename =
        "LT1_VirtualPatient_" + this.selectedModel + "_" + nowString + ".json";
      const content =
