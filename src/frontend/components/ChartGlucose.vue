...
function glucoseColor(ctx) {
  if (ctx.p0.parsed.y < 54 || ctx.p1.parsed.y < 54)
    return "rgb(255,161,161,0.38)"; // very low
  if (ctx.p0.parsed.y < 70 || ctx.p1.parsed.y < 70)
    return "rgb(255,211,211,0.38)"; // low
  if (ctx.p0.parsed.y > 250 || ctx.p1.parsed.y > 250)
    return "rgb(255,207,82,0.38)"; // very high
  if (ctx.p0.parsed.y > 180 || ctx.p1.parsed.y > 180)
    return "rgb(255,229,161,0.38)"; // high
  return "rgb(24,110,168,1)"; // target
}
...
export default {
  ...
  methods: {
    ...
    _pushRecord(t, _x, _u, y, _log) {
      ...
      chartGlucose.data.datasets[this.currentDatasetID].data.push({
        x: t.valueOf(),
        y: y.G,
      });
    },
